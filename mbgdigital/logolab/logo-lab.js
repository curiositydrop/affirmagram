/* MBG Digital — Logo Lab (simple inputs, stable client) */
const ENDPOINT = '/api/logo';        // change if your backend path differs
const IMG_COUNT = 4;
...
function showSoftError(onRetry){
  const box = el(`<div class="soft-error"><div>Our generator is cooling down (traffic spike). Retrying usually fixes it.</div><button class="retry-btn">Retry</button></div>`);
...
function normalizeImages(apiResp){
  const out = [];
  (apiResp?.images || apiResp?.data || []).forEach(img=>{
    if (!img) return;
    if (img.url) out.push(img.url);
    else if (img.b64_json || img.base64) out.push(`data:image/png;base64,${img.b64_json || img.base64}`);
  });
  return out;
}
...
async function generate(){
  ...
  try {
    ...
    if (!urls.length){ showSoftError(()=>{ renderForm(); generate(); }); setStatus('No images yet. Retry usually helps.'); return; }
